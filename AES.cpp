...
Description : AES source code for each mode
...
void AES::SetKey(u8 *key) {
	KeyExpansion(key, w);
}

u8* AES::Cipher(u8* input, u8 *output)
{
	u8 state[4][4];
	int i, r, c;
...
u8* AES::InvCipher(u8* input, u8 *output)
{
	u8 state[4][4];
	int i, r, c;
...
void AES::KeyExpansion(u8* key, u8 w[][4][4])
{
	int i, j, r, c;
	u8 rc[] = { 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36 };
	for (r = 0; r<4; r++)
	{
		for (c = 0; c<4; c++)
		{
...
void AES::MixColumns(u8 state[][4])
{
	u8 t[4];
	int r, c;
	for (c = 0; c< 4; c++)
...
void AES::AddRoundKey(u8 state[][4], u8 k[][4])
{
	int r, c;
	for (c = 0; c<4; c++)
	{
...
void AES::InvSubBytes(u8 state[][4])
{
	int r, c;
	for (r = 0; r<4; r++)
	{
...
void AES::InvShiftRows(u8 state[][4])
{
	u8 t[4];
	int r, c;
	for (r = 1; r<4; r++)
...
void AES::InvMixColumns(u8 state[][4])
{
	u8 t[4];
	int r, c;
	for (c = 0; c< 4; c++)
...
void AESModeOfOperation::set_key(u8 *_key) {
	assert(_key != NULL);
	memcpy(m_key, _key, 16);
	m_aes->SetKey(m_key);
}
void AESModeOfOperation::set_iv(u8 *_iv) {
	assert(_iv != NULL);
	memcpy(m_iv, _iv, 16);
}

int AESModeOfOperation::Encrypt(u8 *_in, int _length, u8 *_out) {
...
int AESModeOfOperation::Decrypt(u8 *_in, int _length, u8 *_out) {
	// TODO :
	bool first_round = true;
	int rounds = 0;
	u8 ciphertext[16] = { 0 };
